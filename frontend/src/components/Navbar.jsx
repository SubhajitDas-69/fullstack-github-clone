...
export default function Navbar({ username, repoName = null, repoId, userId, issueId, commitId, fileName }) {
      ...
      <div className="navDiv">
            ...
            <div id="AppLogo">
              ...
              {username ? (
                <h4>
                  ...
                  <span className="repoNameNav">{repoName}</span>
                </h4>
              ) : (
                <h4>Dashboard</h4>
              )}
        ...
        <div className="navMenu">
          <div className="tooltip">
            <Link to="/create" id="createRepoLink">
              <button className="createRepo">+</button>
              <span className="tooltiptext">Create new...</span>
            </Link>
          </div>

          <div onClick={() => setSidebarOpen(true)} style={{ cursor: "pointer" }}>
            <UserAvatar userId={localStorage.getItem("userId")} cssProfileId={"NavProfile"} />
          </div>
        </div>
      </div>
      <ProfileSidebar isOpen={sidebarOpen} onClose={() => setSidebarOpen(false)} />
        <NavSidebar repo={repo} repoId={repoId} repoName={repoName} issueId={issueId} commitId={commitId} fileName={fileName} />
      ...
      {location.pathname.startsWith("/GitHub") ||
        ...
        ? (
          <div id="header" className="header1">
            <Link
              to={`/GitHub/CurrentUser/${localStorage.getItem("userId")}/repo/${repoId}`}
              className={location.pathname === `/GitHub/CurrentUser/${localStorage.getItem("userId")}/repo/${repoId}` ||
                location.pathname === `/view/${encodeURIComponent(repoName)}/${repoId}/allCommits/${commitId}` ||
                location.pathname === `/GitHub/CurrentUser/${localStorage.getItem("userId")}/repo/${repoId}/commits` ||
                location.pathname === `/fileContent/${repoId}/${commitId}/file/${encodeURIComponent(fileName)}`
                ? "active" : ""}
            >
            ...
            <Link
              to={`/Issues/${repoId}`}
              className={location.pathname === `/Issues/${repoId}` || location.pathname === `/create/issue/${repoId}` ||
                location.pathname === `/viewIssue/${issueId}/repo/${repoId}` ? "active" : ""}
              id="issueBar"
            >
              ...
              <b>Issues
                <span>
                  <span>
                    {repo?.openedIssues?.length > 0 && (
                      <span>{repo?.openedIssues?.length}</span>
                    )}
                  </span>
                </span>
              </b>
            ...
            {repo?.owner?._id === localStorage.getItem("userId") && (
              <Link to={`/${repoId}/settings`}
                className={location.pathname === `/${repoId}/settings` ? "active" : ""}
              >
        ...
        <NavSidebar repo={repo} repoId={repoId} repoName={repoName} issueId={issueId} commitId={commitId} fileName={fileName} userId={userId} user={user}/>
        ...
        ? (
          <div id="header">
            ...
            <Link
              ...
              className={location.pathname === `/Allrepo/user/${userId}` ? "active" : ""}
            >
            ...
            <Link
              to={`/stars/${userId}`}
              className={location.pathname === `/stars/${userId}` ? "active" : ""}
            >
              ...
              <b>Stars
                <span>
                  {user.starRepos?.length > 0 && (
                    <span>{user.starRepos.length}</span>
                  )}
                </span></b>
            </Link>
          </div>
